...
import {
  pizzaDoughButtonArray,
  pizzaSizeButtonArray,
} from "../const";
    ...
    <>
      <div className={cl.button_size_container}>
        {pizzaSizeButtonArray.map(({ id, info }) => (
          <ButtonSizePizzaItem key={id} id={id} active={activeSize} setActive={setActiveSize}>
            {info}
          </ButtonSizePizzaItem>
        ))}
      </div>
      <div className={cl.button_dough_container}>
        {pizzaDoughButtonArray.map(({ info, id }) => (
          <ButtonDoughPizzaItem key={id} setActive={setActiveDough} active={activeDough} id={id}>
            {info}
